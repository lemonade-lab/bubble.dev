...
使用 `bubbleblue://` 协议创建可点击的命令链接。

**语法**:

```
bubbleblue://win/md/input?command={命令}&reply={true|false}&enter={true|false}
```
...
```markdown
[点击查看帮助](bubbleblue://win/md/input?command=/help&reply=false&enter=false)
[发送问候](bubbleblue://win/md/input?command=你好&reply=false&enter=true)
```
